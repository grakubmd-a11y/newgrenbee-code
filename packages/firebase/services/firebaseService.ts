...
import { db, auth, storage } from "@grenbee/firebase";
import { AdminActivityEvent, AreaContent, Booking, BookingStatus, CouponRule, ContactPageContent, FaqPageContent, HomePageContent, MediaItem, MembershipPlan, Review, Service, Staff, Coverage, BusinessSettings, RecurringPlan } from "@grenbee/types";
...
export async function fetchLeadsForAdmin(): Promise<import("@grenbee/types").Lead[]> {
  try {
    const snap = await getDocs(
      query(collection(db, "leads"), orderBy("createdAt", "desc"))
    );
    return snap.docs.map((d) => ({ id: d.id, ...d.data() }) as import("@grenbee/types").Lead);
  } catch {
    return [];
  }
}
...
export async function fetchContactSubmissions(): Promise<import("@grenbee/types").ContactSubmission[]> {
  try {
    const snap = await getDocs(
      query(collection(db, "contactSubmissions"), orderBy("createdAt", "desc"))
    );
    return snap.docs.map((d) => d.data() as import("@grenbee/types").ContactSubmission);
  } catch {
    return [];
  }
}
